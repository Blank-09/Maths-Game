...
start_btn.onclick = ()=>{name_box.classList.add("activeName")}
leaderboard.onclick = ()=>{document.getElementById("leaderboard_link").click()}
...
function createQuestions(){
  ...
  for(var i=0; i<4; i++){
    ...
    document.querySelectorAll(".option")[i].setAttribute("onclick", "optionSelected(this)"); 
  }
}
...
function encript(){
  ...
  arr = ["w","P","f", "R", "i", "Y","A", "a", "g","N", "q", "s", "U", "m","T"]
  ...
  var s = randomAlpha();
  var q = randomAlpha();
  link = "https://api.whatsapp.com/send?phone=918072381972&text=Name%3A%20"+username+"%0Acode%3A%20*"+r+encript.join('')+s+q+"*%0AScore%3A%20"+userScore;
